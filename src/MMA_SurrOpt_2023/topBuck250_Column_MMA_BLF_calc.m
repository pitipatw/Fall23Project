%%writefile topBuck250_Column_MMA_BLF_calc.m
function [blf,xpOut_reshape] = topBuck250_Column_MMA_BLF_calc(nelx,nely,penalK,rmin,ft,ftBC,eta,beta,ocPar,maxit,Lx,penalG,nEig,pAgg,prSel,x0,betaCnt,asyincr_inp,asydecr_inp)
% ---------------------------- PRE. 1) MATERIAL AND CONTINUATION PARAMETERS

% addpath('MMA_SurrOpt_2023')
...
lcDof = 2*nodeNrs(nely/2+1+[-4:4],end)-1;                                  % loaded DOFs
modF = 1e-3/Ly/(length(lcDof)-1);                                          % modulus of the force density
%CONVERT FROM FSPARSE TO SPARSE
F = sparse(double(lcDof),ones(length(lcDof),1),-modF*ones(length(lcDof),1),nDof,1);
[F(lcDof(1)),F(lcDof(end))] = deal(F(lcDof(1))/2,F(lcDof(end))/2);         % consistent load on end nodes
[pasS,pasV] = deal(elNrs(nely/2+[-4:5],end-4:end),[]);                    % define passive domains
...
if nargin > 15
    % x =  csvread("x0.csv");                                                % initialize design from saved data
    %here, treat x0 as a file name. 
    x = csvread(strcat(x0, ".csv"));
else
...
n     = length(act);             % The number of design variables x_j.
xmin  = zeros(length(act),1);       % Column vector with the lower bounds for the variables x_j.
xmax  = ones(length(act),1);        % Column vector with the upper bounds for the variables x_j.
xOld  = x(:);             % xval, one iteration ago (provided that iter>1).
xOld1 = x(:);             % xval, two iterations ago (provided that iter>2).
low   = ones(length(act),1);        % Column vector with the lower asymptotes from the previous iteration (provided that iter>1).
upp   = ones(length(act),1);        % Column vector with the upper asymptotes from the previous iteration (provided that iter>1).
a0MMA = 1;                % The constants a_0 in the term a_0*z.
aMMA  = zeros(m,1);       % Column vector with the constants a_i in the terms a_i*z.
ccMMA = 10000*ones(m,1);  % Column vector with the constants c_i in the terms c_i*y_i.
ddMMA = zeros(m,1);       % Column vector with the constants d_i in the terms 0.5*d_i*(y_i)^2.
...
%   end
  ...
  G = sparse(IkG(:,1)+1,IkG(:,2)+1,sG,nDof,nDof)+...
         sparse(IkG(:,1),  IkG(:,2),  sG,nDof,nDof);                    % assemble global G matrix
  ...
  if loop==1, xOld = x(act); xOld1 = xOld; low = []; upp = []; end
  [xMMA,yy,lmid,xi,eeta,muu,zet,s1,low,upp] = mmasub(1,length(act),loop,x(act),xmin,...
  xmax,xOld,xOld1,g0,dg0(act),g1,dg1(act)',low,upp,a0MMA,aMMA,ccMMA,ddMMA,beta,asyincr_inp,asydecr_inp);
...
blf = 1./muVec(end,1);
xpOut = xPhys;
xpOut_reshape = reshape(xpOut,nely,nelx);
